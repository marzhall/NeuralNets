...
module Node =
    ...
    type Node(rnd:System.Random) =
        let getRandomFloats count = 
            Array.init count (fun _ -> rnd.NextDouble())
        ...
        member this.mutate(learningRate : double) =
            ...
            let mutatedBias = this.bias + randomNegative(rnd.NextDouble()*learningRate)
            let mutatedWeights = Array.map (fun x -> x + (randomNegative (rnd.NextDouble()*learningRate))) this.weights
